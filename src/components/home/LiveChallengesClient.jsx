...
const LIVE_CHALLENGE_FILTERS = {
  isChallenge: 'true',
  status: 'upcoming,registration_open',
  limit: 6,
};

export default function LiveChallengesClient({ initialChallenges = [], pollInterval = 60000 }) {
  const [challenges, setChallenges] = useState(initialChallenges);
  const [loading, setLoading] = useState(!initialChallenges.length);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const data = await getMatches(LIVE_CHALLENGE_FILTERS);
        if (!isMounted) return;
    ...
    // Fetch once after hydration to keep data fresh, then poll at the given interval
    refresh();
    const intervalId = pollInterval ? setInterval(refresh, pollInterval) : null;
  ...
  const showSkeleton = loading && !challenges.length;
        ...
        {showSkeleton ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {challenges.map((challenge, index) => (
              <Link
                key={challenge._id || `${challenge.title}-${index}`}
